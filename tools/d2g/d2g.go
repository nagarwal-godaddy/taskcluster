...
func runCommand(
	...
	// Add environment variables with newlines
	if envVarsWithNewlines != "" {
...
func createVolumeMountArgs(
	...
	for _, wdc := range wdcs {
		args = append(args, "-v", fmt.Sprintf("$(pwd)/%s:%s", wdc.Directory, dwPayload.Cache[wdc.CacheName]))
	}
	for _, gwArtifact := range gwArtifacts {
		if strings.HasPrefix(gwArtifact.Path, "volume") {
			args = append(args, "-v", fmt.Sprintf("$(pwd)/%s:%s", gwArtifact.Path, dwPayload.Artifacts[gwArtifact.Name].Path))
		}
	}
